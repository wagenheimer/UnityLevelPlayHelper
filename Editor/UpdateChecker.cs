...
namespace Wagenheimer.LevelPlayHelper.Editor
{
    ...
    {
        const string PackageDisplayName = "Level Play Helper";
        internal const string GitUrl = "https://github.com/wagenheimer/UnityLevelPlayHelper.git";
        const string PackageJsonUrl = "https://raw.githubusercontent.com/wagenheimer/UnityLevelPlayHelper/master/package.json";
        const string ChangelogUrl = "https://raw.githubusercontent.com/wagenheimer/UnityLevelPlayHelper/master/CHANGELOG.md";
        const string RepoUrl = "https://github.com/wagenheimer/UnityLevelPlayHelper";
        const string PrefLastCheckTicks = "Wagenheimer.LevelPlayHelper.UpdateChecker.LastCheckTicks";
        const string PrefSkipVersion = "Wagenheimer.LevelPlayHelper.UpdateChecker.SkipVersion";
        ...
        [MenuItem("Tools/Wagenheimer/Level Play Helper/Check for Updates...", priority = 141)]
        ...
        static void OnPackageJsonComplete(UnityWebRequest request, bool force)
        {
            ...
            {
                Debug.Log($"[LevelPlayHelper] Update check failed: {request.error}");
                if (force)
            ...
            {
                Debug.Log($"[LevelPlayHelper] Update check failed: could not parse remote package.json ({e.Message})");
            }
            ...
            {
                Debug.Log("[LevelPlayHelper] Update check failed: remote package.json has no version field.");
                if (force)
            ...
            {
                Debug.Log("[LevelPlayHelper] Update check failed: could not resolve the installed package version " +
            ...
            {
                Debug.Log($"[LevelPlayHelper] Up to date (installed: {localVersion}).");
                if (force)
            ...
            {
                Debug.Log($"[LevelPlayHelper] Version {remoteVersion} available (installed: {localVersion}) but ignored by user preference.");
                return;
            }

            Debug.Log($"[LevelPlayHelper] New version available: {remoteVersion} (installed: {localVersion}). See {RepoUrl}/releases/latest");
